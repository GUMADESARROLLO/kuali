            ...
            <table class="w-full text-left min-w-[500px]">
                <thead>
                    <tr class="bg-surface-container-low dark:bg-gray-700 border-b border-border-subtle dark:border-gray-700">
                        ...
                        <th class="px-6 py-3 text-label-sm uppercase text-outline dark:text-gray-400 font-bold tracking-wider hidden sm:table-cell">Email</th>
                        <th class="px-6 py-3 text-label-sm uppercase text-outline dark:text-gray-400 font-bold tracking-wider hidden md:table-cell">Tel&eacute;fono</th>
                ...
                <tbody class="divide-y divide-border-subtle dark:divide-gray-700">
                    <tr v-for="p in persons.data" :key="p.id" class="hover:bg-surface-container-lowest dark:hover:bg-gray-700/50">
                        ...
                        <td class="px-6 py-3 text-body-sm text-outline dark:text-gray-400 hidden sm:table-cell">{{ p.email || '—' }}</td>
                        <td class="px-6 py-3 text-body-sm text-outline dark:text-gray-400 hidden md:table-cell">{{ p.phone || '—' }}</td>
                        <td class="px-6 py-3 text-body-sm text-outline dark:text-gray-400 hidden sm:table-cell">{{ p.company?.name ?? '—' }}</td>
                    ...
                    <tr v-if="!persons.data.length"><td colspan="6" class="py-12 text-center text-outline dark:text-gray-400 text-body-sm">Sin personas registradas.</td></tr>
                </tbody>
            </table>
            </div>
            <PaginationBar :links="persons.links" :from="persons.from" :to="persons.to" :total="persons.total" />
        </div>
