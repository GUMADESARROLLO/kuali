...
const typeClass = (t: string) => t === 'reparacion' ? 'bg-red-100 text-red-700 dark:bg-red-900/30 dark:text-red-400' : t === 'reemplazo' ? 'bg-yellow-100 text-yellow-700 dark:bg-yellow-900/30 dark:text-yellow-400' : 'bg-blue-100 text-blue-700 dark:bg-blue-900/30 dark:text-blue-400';
</script>
    ...
    <AppLayout active-nav="maintenance">
        ...
        <div class="bg-white dark:bg-gray-800 rounded-xl border border-border-subtle dark:border-gray-700 shadow-sm overflow-visible flex flex-col">
            <div class="p-4 flex flex-col sm:flex-row items-stretch sm:items-center gap-3 bg-surface-container-lowest dark:bg-gray-800/50 border-b border-border-subtle dark:border-gray-700">
                <select v-model="assetFilter" class="w-full sm:w-auto border-border-subtle dark:border-gray-600 dark:bg-gray-700 dark:text-gray-200 rounded-lg text-label-sm py-2 px-3">
                    <option value="">Todos los activos</option>
                    <option v-for="a in assets" :key="a.id" :value="a.id">{{ a.asset_tag }} — {{ a.name }} ({{ a.company?.name || '?' }})</option>
                </select>
                <div class="flex items-center gap-2 w-full sm:w-auto">
                    <select v-model="typeFilter" class="flex-1 sm:w-auto border-border-subtle dark:border-gray-600 dark:bg-gray-700 dark:text-gray-200 rounded-lg text-label-sm py-2 px-3">
            ...
            <table class="w-full text-left min-w-[600px]">
                <thead>
                    <tr class="bg-surface-container-low dark:bg-gray-700 border-b border-border-subtle dark:border-gray-700">
                        ...
                        <th class="px-6 py-3 text-label-sm uppercase text-outline dark:text-gray-400 font-bold tracking-wider">Tipo</th>
                        <th class="px-6 py-3 text-label-sm uppercase text-outline dark:text-gray-400 font-bold tracking-wider hidden sm:table-cell">Componente</th>
                        <th class="px-6 py-3 text-label-sm uppercase text-outline dark:text-gray-400 font-bold tracking-wider">Descripción</th>
                        <th class="px-6 py-3 text-label-sm uppercase text-outline dark:text-gray-400 font-bold tracking-wider hidden md:table-cell">Costo</th>
                        <th class="px-6 py-3 text-label-sm uppercase text-outline dark:text-gray-400 font-bold tracking-wider hidden sm:table-cell">Fecha</th>
                ...
                <tbody class="divide-y divide-border-subtle dark:divide-gray-700">
                    <tr v-for="r in records.data" :key="r.id" class="hover:bg-surface-container-lowest dark:hover:bg-gray-700/50">
                        ...
                        <td class="px-6 py-3 text-body-sm text-outline dark:text-gray-400 hidden sm:table-cell">{{ r.component || '—' }}</td>
                        <td class="px-6 py-3 text-body-sm text-outline dark:text-gray-400 max-w-[120px] sm:max-w-xs truncate">{{ r.description }}</td>
                        <td class="px-6 py-3 text-body-sm text-outline dark:text-gray-400 hidden md:table-cell">{{ r.cost || '—' }}</td>
                        <td class="px-6 py-3 text-body-sm text-outline dark:text-gray-400 hidden sm:table-cell">{{ new Date(r.performed_at).toLocaleDateString('es') }}</td>
                        <td class="px-6 py-3"><div class="flex items-center justify-center"><button @click="destroy(r.id)" class="p-1.5 text-on-surface-variant dark:text-gray-300 hover:text-error hover:bg-error-container/20 dark:hover:bg-red-900/30 rounded" title="Eliminar"><span class="material-symbols-outlined text-[18px]">delete</span></button></div></td>
                    </tr>
                    <tr v-if="!records.data.length"><td colspan="7" class="py-12 text-center text-outline dark:text-gray-400 text-body-sm">Sin registros.</td></tr>
                </tbody>
            </table>
            </div>
            <PaginationBar :links="records.links" :from="records.from" :to="records.to" :total="records.total" />
        </div>
    </AppLayout>
</template>
